...
const quizQuestions = Object.values(quizData)
  ...
  .map((item, index) => {
    ...
    return baseQuestion({
      ...
      status: 'validated',
      approvals: [
        { reviewer: 'Lucas', at: generatedAt },
        { reviewer: 'Awen', at: generatedAt },
      ],
      tags: ['socle', 'quiz'],
      source: 'Socle',
      sourcePage: '',
...
const csvRows = questions.map((item) => ({
  ...
  lucas_validation: item.approvals.find((approval) => approval.reviewer === 'Lucas')?.at || '',
  awen_validation: item.approvals.find((approval) => approval.reviewer === 'Awen')?.at || '',
